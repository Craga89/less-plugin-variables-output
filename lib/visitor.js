...
class VariablesExportVisitor {
  ...
  run(root) {
    ...
    const variables = collectVariables(root.originalRuleset)
      .reduce((acc, variable) => {
        const { currentFileInfo, name: key, value } = variable;
        // make it windows compatible
        const filename = currentFileInfo.filename.replace('\\', '/');
        const rootFilename = currentFileInfo.rootFilename.replace('\\', '/');
    ...
    Object.entries(variables).forEach(([filename, fileVariables]) => {
      // eval values
      const json = {};
      Object.entries(fileVariables).forEach(([moduleName, moduleVariables]) => {
        json[moduleName] = {};
